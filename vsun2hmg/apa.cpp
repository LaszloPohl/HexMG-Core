...
//***********************************************************************
void apa::write_HMG_akt_sim(FILE *fp, simulation & aktSim){
//***********************************************************************

    fprintf(fp, "HEXMG 1.0\n\n");

    // write RAILS

    fprintf(fp, ".RAILS=2 R1=%g\n\n", aktSim.ambiT);

    write_HMG_material_functions(fp, aktSim);
    write_HMG_boundary_cell_models(fp, aktSim);
    write_HMG_colors(fp, aktSim);
    write_HMG_junctions(fp, aktSim);
    write_HMG_SUNRED_tree(fp, aktSim);
    write_HMG_cell_models(fp, aktSim);

    fprintf(fp, ".CREATE circ1 main GND=R%u\n\n", aktSim.mezo == FieldTherm ? 1 : 0);
...
//***********************************************************************
void apa::write_HMG_material_functions(FILE *fp, simulation & aktSim){
// az apa::write_materials()-ból
//***********************************************************************
    for (uns i = 0; i < aktSim.pmodel->tmat.size(); i++)
        if (aktSim.pmodel->tmat[i].is_isotrop()) {
            aktSim.pmodel->tmat[i].hmg_write_nonlin(fp);
        }
        else {
            throw hiba("apa::write_HMG_material_functions", "!is_isotrop not implemented");
        }
}


//***********************************************************************
void apa::write_HMG_boundary_cell_models(FILE *fp, simulation & aktSim){
//***********************************************************************
    fprintf(fp, "* boundaries:\n");
    write_HMG_boundary_global_vars(fp, aktSim, aktSim.normalperem, WEST,   "WEST");
    write_HMG_boundary_global_vars(fp, aktSim, aktSim.normalperem, EAST,   "EAST");
    write_HMG_boundary_global_vars(fp, aktSim, aktSim.normalperem, SOUTH,  "SOUTH");
    write_HMG_boundary_global_vars(fp, aktSim, aktSim.normalperem, NORTH,  "NORTH");
    write_HMG_boundary_global_vars(fp, aktSim, aktSim.normalperem, BOTTOM, "BOTTOM");
    write_HMG_boundary_global_vars(fp, aktSim, aktSim.normalperem, TOP,    "TOP");
    for (uns i = 0; i < aktSim.tinner.size(); i++)
        write_HMG_boundary_global_vars(fp, aktSim, aktSim.tinner[i], WEST, PLString("INNER") + i); // minden oldalon ugyanaz kell legyen
}
...
//***********************************************************************
    ...
    if (csom.th[index].is_special)
        throw hiba("apa::write_HMG_boundary_global_vars", "th special boubdary condition not implemented");
    if (aktSim.mezo != FieldTherm)
        switch (csom.el[index].tipus) {
            ...
            case PeremV:
                fprintf(fp, ".SET BG_%s_Ge %g\n", name.c_str(), GV);
                fprintf(fp, ".SET BG_%s_Ie %g\n", name.c_str(), GV * csom.el[index].value);
                break;
            default:
                throw hiba("apa::write_HMG_boundary_global_vars", "unknown el boubdary condition (%u)", csom.el[index].tipus);
        }
    if (aktSim.mezo != FieldEl)
        switch (csom.th[index].tipus) {
            case PeremOpen:
...
/*
    ...
    fprintf(fp, ".MODEL BOU_EL_%s SUBCIRCUIT X=1 N=0 P=0\n", name.c_str());
    fprintf(fp, "G0 G X0 R0 BG%u\n", (uns)hmg_global_var_num);
    fprintf(fp, "I0 I X0 R0 BG%u BG%u\n", (uns)(hmg_global_var_num + 1), (uns)(hmg_global_var_num + 1));
    fprintf(fp, ".END MODEL BOU_EL_%s\n\n", name.c_str());

    fprintf(fp, ".MODEL BOU_TH_%s SUBCIRCUIT X=1 N=0 P=1\n", name.c_str());
    fprintf(fp, "G0 G2 X0 R0 BG%u P0\n", (uns)(hmg_global_var_num + 2));
    fprintf(fp, "G1 G X0 R0 BG%u\n", (uns)(hmg_global_var_num + 3));
    fprintf(fp, "I0 I X0 R0 BG%u BG%u\n", (uns)(hmg_global_var_num + 4), (uns)(hmg_global_var_num + 4));
    fprintf(fp, ".END MODEL BOU_TH_%s\n\n", name.c_str());

    fprintf(fp, ".MODEL BOU_ELTH_%s SUBCIRCUIT X=2 N=0 P=1\n", name.c_str());
    fprintf(fp, "G0 G X0 R0 BG%u\n", (uns)hmg_global_var_num);
    fprintf(fp, "I0 I X0 R0 BG%u BG%u\n", (uns)(hmg_global_var_num + 1), (uns)(hmg_global_var_num + 1));
    fprintf(fp, "G1 G2 X1 R0 BG%u P0\n", (uns)(hmg_global_var_num + 2));
    fprintf(fp, "G2 G X1 R0 BG%u\n", (uns)(hmg_global_var_num + 3));
    fprintf(fp, "I1 I X1 R0 BG%u BG%u\n", (uns)(hmg_global_var_num + 4), (uns)(hmg_global_var_num + 4));
    fprintf(fp, ".END MODEL BOU_ELTH_%s\n\n", name.c_str());
*/
}
...
//*********************************************************************** 
    ...
    */
    ...
    for (uns i = 0; i < colmax; i++)
        if (aktSim.pmodel->tcolor[i].is && aktSim.pmodel->tcolor[i].tipus == SzinNormal && aktSim.pmodel->tcolor[i].terfogat>0.0) {
            if (aktSim.mezo != FieldTherm) fprintf(fp, ".SET BG_COLOR%u_Gelx 0\n", i);  // Gex => a gerjesztõ forrás vezetése (áramnál 0, fesznél nagy)
            if (aktSim.mezo != FieldTherm) fprintf(fp, ".SET BG_COLOR%u_Ielxi 0\n", i); // Iiex => ha áram gerjesztés van, akkor térfogatarányosan kell a cellákat gerjeszteni, ezért ehhez egy kétparaméteres áramforrás tartozik
            if (aktSim.mezo != FieldTherm) fprintf(fp, ".SET BG_COLOR%u_Ielxv 0\n", i); // Ivex => ha fesz gerjesztés van, akkor minden cellára ugyanaz a gerjesztés megy, ezért egy másik, egyparaméteres áramforrás van
            if (aktSim.mezo != FieldEl) fprintf(fp, ".SET BG_COLOR%u_Gthx 0\n", i);  // Gex => a gerjesztõ forrás vezetése (áramnál 0, fesznél nagy)
            if (aktSim.mezo != FieldEl) fprintf(fp, ".SET BG_COLOR%u_Ithxi 0\n", i); // Iiex => ha áram gerjesztés van, akkor térfogatarányosan kell a cellákat gerjeszteni, ezért ehhez egy kétparaméteres áramforrás tartozik
            if (aktSim.mezo != FieldEl) fprintf(fp, ".SET BG_COLOR%u_Ithxv 0\n", i); // Ivex => ha fesz gerjesztés van, akkor minden cellára ugyanaz a gerjesztés megy, ezért egy másik, egyparaméteres áramforrás van
        }
    fprintf(fp, "\n");
}
    ...
    for (uns i = 0; i < colmax; i++)
        if (aktSim.pmodel->tcolor[i].is && aktSim.pmodel->tcolor[i].tipus == SzinNormal && aktSim.pmodel->tcolor[i].terfogat>0.0) {
            for (uns j = 0; j < aktSim.pmodel->tcolor[i].tsemi.size(); j++) {
                ...
                aktSim.pmodel->tcolor[i].tsemi[j].par.hmg_write_nonlinfunction(fp, hnctSemiEq, aktSim.pmodel->tcolor[i].tsemi[j].As, true);
                aktSim.pmodel->tcolor[i].tsemi[j].D.hmg_write_nonlinfunction(fp, hnctSemiD, aktSim.pmodel->tcolor[i].tsemi[j].As);
                aktSim.pmodel->tcolor[i].tsemi[j].R.hmg_write_nonlinfunction(fp, hnctSemiR, aktSim.pmodel->tcolor[i].tsemi[j].As);
                aktSim.pmodel->tcolor[i].tsemi[j].rad.hmg_write_nonlinfunction(fp, hnctSemiRad, aktSim.pmodel->tcolor[i].tsemi[j].As, true);
                aktSim.pmodel->tcolor[i].tsemi[j].lum.hmg_write_nonlinfunction(fp, hnctSemiLum, aktSim.pmodel->tcolor[i].tsemi[j].As, true);
            }
        }
}
...
//***********************************************************************
    ...
    struct Level {
        bool is_levvel_exists = false;
        size_t real_level_index = 0;
        tomb3d<cell_red> grid;
    };
    ...
    levels[0].is_levvel_exists = true;
    levels[0].grid.resize(x_res, y_res, z_res);
    uns index = 0;
    ...
    for (uns z = 0; z < z_res; z++)
        for (uns y = 0; y < y_res; y++)
            for (uns x = 0; x < x_res; x++) {
                ...
                cell_red& cell = levels[0].grid.getref(x, y, z);

                if (cell.is_exists = cella_color.tipus == SzinNormal) {
                    cell.src1_index = cell.index = index++;
                    cell.src1_level = 0;
                    cell.is_reduction = false;
                }
    ...
    while (true) {
        ...
        if (x_size == 1 && y_size == 1 && z_size == 1)
            break;
        if (x_size > 1) {
            x_size = x_size > 2 ? (x_size + 3) / 4 + (x_size + 2) / 4 : 1;
            levels.resize(levels.size() + 1);
            Level& srclevel = levels[levels.size() - 2];
            Level& destlevel = levels[levels.size() - 1];
            tomb3d<cell_red>& srcgrid = srclevel.grid;
            tomb3d<cell_red>& destgrid = destlevel.grid;
            destgrid.resize(x_size, y_size, z_size);
            bool is_red = false;
            uns dest_index = 0;

            for (uns z = 0; z < srcgrid.z_size(); z++)
                for (uns y = 0; y < srcgrid.y_size(); y++)
                    for (uns x = 0, dx = 0; x < srcgrid.x_size(); dx++) {

                        bool is_double = (x != srcgrid.x_size() - 1);
                        if (is_double && srcgrid.x_size() > 2 && (srcgrid.x_size() + 1) / 2 - 1 == x) // ha a felénél 1 cellasor van
                            is_double = false;

                        cell_red& dest_cell = destgrid.getref(dx, y, z);

                        if (is_double) {
                            cell_red& src1_cell = srcgrid.getref(x, y, z);
                            cell_red& src2_cell = srcgrid.getref(x + 1, y, z);
                            if (!src1_cell.is_exists && !src2_cell.is_exists) {
                                dest_cell.is_exists = false;
                            }
                            else if (!src1_cell.is_exists) {
                                dest_cell = src2_cell;
                                dest_cell.is_reduction = false;
                                if (src2_cell.is_reduction) {
                                    dest_cell.src1_level = levels.size() - 2;
                                    dest_cell.src1_index = src2_cell.index;
                                }
                            }
                            else if (!src2_cell.is_exists) {
                                dest_cell = src1_cell;
                                dest_cell.is_reduction = false;
                                if (src1_cell.is_reduction) {
                                    dest_cell.src1_level = levels.size() - 2;
                                    dest_cell.src1_index = src1_cell.index;
                                }
                            }
                            else { // mindkét forráscella létezik
                                is_red = true;
                                dest_cell.is_exists = true;
                                dest_cell.is_reduction = true;
                                if (src1_cell.is_reduction) {
                                    dest_cell.src1_level = levels.size() - 2;
                                    dest_cell.src1_index = src1_cell.index;
                                }
                                else {
                                    dest_cell.src1_level = src1_cell.src1_level;
                                    dest_cell.src1_index = src1_cell.src1_index;
                                }
                                if (src2_cell.is_reduction) {
                                    dest_cell.src2_level = levels.size() - 2;
                                    dest_cell.src2_index = src2_cell.index;
                                }
                                else {
                                    dest_cell.src2_level = src2_cell.src1_level;
                                    dest_cell.src2_index = src2_cell.src1_index;
                                }
                                dest_cell.index = dest_index++;
                            }
                            x += 2;
                        }
                        else {
                            cell_red& src_cell = srcgrid.getref(x, y, z);
                            dest_cell = src_cell;
                            if (src_cell.is_exists) {
                                dest_cell.is_reduction = false;
                                if (src_cell.is_reduction) {
                                    dest_cell.src1_level = levels.size() - 2;
                                    dest_cell.src1_index = src_cell.index;
                                }
                            }
                            x++;
                        }
                    }
            destlevel.is_levvel_exists = is_red;
        }
        ...
        if (x_size == 1 && y_size == 1 && z_size == 1)
            break;
        if (y_size > 1) {
            y_size = y_size > 2 ? (y_size + 3) / 4 + (y_size + 2) / 4 : 1;
            levels.resize(levels.size() + 1);
            Level& srclevel = levels[levels.size() - 2];
            Level& destlevel = levels[levels.size() - 1];
            tomb3d<cell_red>& srcgrid = srclevel.grid;
            ...
            for (uns z = 0; z < srcgrid.z_size(); z++)
                for (uns y = 0, dy = 0; y < srcgrid.y_size(); dy++) {

                    bool is_double = (y != srcgrid.y_size() - 1);
                    if (is_double && srcgrid.y_size() > 2 && (srcgrid.y_size() + 1) / 2 - 1 == y) // ha a felénél 1 cellasor van
                        is_double = false;

                    for (uns x = 0; x < srcgrid.x_size(); x++) {

                        cell_red& dest_cell = destgrid.getref(x, dy, z);

                        if (is_double) {
                            cell_red& src1_cell = srcgrid.getref(x, y, z);
                            cell_red& src2_cell = srcgrid.getref(x, y + 1, z);
                            if (!src1_cell.is_exists && !src2_cell.is_exists) {
                                dest_cell.is_exists = false;
                            }
                            else if (!src1_cell.is_exists) {
        ...
        if (x_size == 1 && y_size == 1 && z_size == 1)
            break;
        if (z_size > 1) {
            z_size = z_size > 2 ? (z_size + 3) / 4 + (z_size + 2) / 4 : 1;
            levels.resize(levels.size() + 1);
            Level& srclevel = levels[levels.size() - 2];
            Level& destlevel = levels[levels.size() - 1];
            tomb3d<cell_red>& srcgrid = srclevel.grid;
            ...
            for (uns z = 0, dz = 0; z < srcgrid.z_size(); dz++) {

                bool is_double = (z != srcgrid.z_size() - 1);
                if (is_double && srcgrid.z_size() > 2 && (srcgrid.z_size() + 1) / 2 - 1 == z) // ha a felénél 1 cellasor van
                    is_double = false;

                for (uns y = 0; y < srcgrid.y_size(); y++)
                    for (uns x = 0; x < srcgrid.x_size(); x++) {

                        cell_red& dest_cell = destgrid.getref(x, y, dz);

                        if (is_double) {
                            cell_red& src1_cell = srcgrid.getref(x, y, z);
                            cell_red& src2_cell = srcgrid.getref(x, y, z + 1);
                            if (!src1_cell.is_exists && !src2_cell.is_exists) {
                                dest_cell.is_exists = false;
                            }
                            else if (!src1_cell.is_exists) {
    ...
    for (size_t i = 0, real_level = 0; i < levels.size(); i++)
        if (levels[i].is_levvel_exists)
            levels[i].real_level_index = real_level++;
        else
            levels[i].real_level_index = real_level;

    fprintf(fp, ".SUNREDTREE TREE\n");

    for (size_t i = 1; i < levels.size(); i++) {
        if (levels[i].is_levvel_exists) {
            for (size_t z = 0; z < levels[i].grid.z_size(); z++)
                for (size_t y = 0; y < levels[i].grid.y_size(); y++)
                    for (size_t x = 0; x < levels[i].grid.x_size(); x++) {
                        cell_red& cell = levels[i].grid.getref((uns)x, (uns)y, (uns)z);
                        if (cell.is_exists && cell.is_reduction) {
                            fprintf(fp, "RED %u %u %u %u %u %u\n",
                                (uns)levels[i].real_level_index,
                                (uns)cell.index,
                                (uns)levels[cell.src1_level].real_level_index,
                                (uns)cell.src1_index,
                                (uns)levels[cell.src2_level].real_level_index,
                                (uns)cell.src2_index
                                );
                        }
                    }
    ...
    fprintf(fp, ".END SUNREDTREE TREE\n\n");
}
...
//***********************************************************************
    ...
    cuns y_res = aktMod.y_res;
    cuns z_res = aktMod.z_res;
    hmg_modell_racs.resize(x_res, y_res, z_res);
    ...
    for (uns z = 0; z < z_res; z++)
        for (uns y = 0; y < y_res; y++)
            for (uns x = 0; x < x_res; x++) {
                ...
                cella.core.type = cella_color.tipus;
                hmg_modell_racs.getref(x, y, z).cella_vector_index = ~0;
                if (cella_color.tipus != SzinNormal) { // a channel típust nem kezeljük
                    continue;
                }

                cella.core.pmat = cella_color.pmat;
                cella.core.color_index = cella_szin; // cella_color.index;
                cella.core.x_size = aktMod.x_pit[x].get(nulla);
                cella.core.y_size = aktMod.y_pit[y].get(nulla);
                cella.core.z_size = aktMod.z_pit[z].get(aktMod.x_hossz[2 * x]);
                cella.core.is_el = (cella_color.field != FieldTherm) && (aktSim.mezo != FieldTherm);
                cella.core.is_th = (cella_color.field != FieldEl) && (aktSim.mezo != FieldEl);
                if(!cella.core.is_el && !cella.core.is_th)
                    throw hiba("apa::write_HMG_cell_models", "material field and color field disjunct");
                ...
                for (uns i = WEST; i < BASIC_SIDES; i++) {
                    if (cella.core.is_el && cella.el_out_nodes[i].coreResistor.is_X) {
                        cella.el_out_nodes[i].coreResistor.set(true, cella.xNodes);
                        cella.el_out_nodes[i].boundary.set(true, cella.xNodes);
                        cella.xNodes++;
                    }
                    if (cella.core.is_th && cella.th_out_nodes[i].coreResistor.is_X) {
                        cella.th_out_nodes[i].coreResistor.set(true, cella.xNodes);
                        cella.th_out_nodes[i].boundary.set(true, cella.xNodes);
                        cella.xNodes++;
                    }
                }
                ...
                for (size_t i = 0; i < hmg_cella_vector.size(); i++)
                    if (hmg_cella_vector[i] == cella) {
                        cvi = i;
                        break;
                    }
                if (cvi == hmg_cella_vector.size()) {
                    hmg_cella_vector.push_back(cella);
                    cella.write(fp, aktSim, (uns)cvi);
                }

                hmg_modell_racs.getref(x, y, z).cella_vector_index = (uns)cvi;
    ...
    tomb3d<gridcell> nodegrid;
    nodegrid.resize(x_res, y_res, z_res);
            ...
            for (uns y = 0; y < y_res; y++)
                for (uns x = 0; x < x_res; x++)
                    if (hmg_modell_racs.getref(x, y, z).cella_vector_index != ~0) {
                        ...
                        if (curr_cell.core.is_el && !curr_cell.el_out_nodes[WEST].is_boundary && !curr_cell.el_out_nodes[WEST].is_field_change) {
                            if (nodegrid.getref(x - 1, y, z).E.el == ~0)
                                nodegrid.getref(x - 1, y, z).E.el = curr_node_index++;
                            if (i == 1)
                                fprintf(fp, " N%u", nodegrid.getref(x - 1, y, z).E.el);
                        }
                        if (curr_cell.core.is_th && !curr_cell.th_out_nodes[WEST].is_boundary && !curr_cell.th_out_nodes[WEST].is_field_change) {
                        ...
                        if (curr_cell.core.is_el && !curr_cell.el_out_nodes[EAST].is_boundary && !curr_cell.el_out_nodes[EAST].is_field_change) {
                            if (nodegrid.getref(x, y, z).E.el == ~0)
                                nodegrid.getref(x, y, z).E.el = curr_node_index++;
                            if (i == 1)
                                fprintf(fp, " N%u", nodegrid.getref(x, y, z).E.el);
                        }
                        if (curr_cell.core.is_th && !curr_cell.th_out_nodes[EAST].is_boundary && !curr_cell.th_out_nodes[EAST].is_field_change) {
                        ...
                        if (curr_cell.core.is_el && !curr_cell.el_out_nodes[SOUTH].is_boundary && !curr_cell.el_out_nodes[SOUTH].is_field_change) {
                            if (nodegrid.getref(x, y - 1, z).N.el == ~0)
                                nodegrid.getref(x, y - 1, z).N.el = curr_node_index++;
                            if (i == 1)
                                fprintf(fp, " N%u", nodegrid.getref(x, y - 1, z).N.el);
                        }
                        if (curr_cell.core.is_th && !curr_cell.th_out_nodes[SOUTH].is_boundary && !curr_cell.th_out_nodes[SOUTH].is_field_change) {
                        ...
                        if (curr_cell.core.is_el && !curr_cell.el_out_nodes[NORTH].is_boundary && !curr_cell.el_out_nodes[NORTH].is_field_change) {
                            if (nodegrid.getref(x, y, z).N.el == ~0)
                                nodegrid.getref(x, y, z).N.el = curr_node_index++;
                            if (i == 1)
                                fprintf(fp, " N%u", nodegrid.getref(x, y, z).N.el);
                        }
                        if (curr_cell.core.is_th && !curr_cell.th_out_nodes[NORTH].is_boundary && !curr_cell.th_out_nodes[NORTH].is_field_change) {
                        ...
                        if (curr_cell.core.is_el && !curr_cell.el_out_nodes[BOTTOM].is_boundary && !curr_cell.el_out_nodes[BOTTOM].is_field_change) {
                            if (nodegrid.getref(x, y, z - 1).T.el == ~0)
                                nodegrid.getref(x, y, z - 1).T.el = curr_node_index++;
                            if (i == 1)
                                fprintf(fp, " N%u", nodegrid.getref(x, y, z - 1).T.el);
                        }
                        if (curr_cell.core.is_th && !curr_cell.th_out_nodes[BOTTOM].is_boundary && !curr_cell.th_out_nodes[BOTTOM].is_field_change) {
                        ...
                        if (curr_cell.core.is_el && !curr_cell.el_out_nodes[TOP].is_boundary && !curr_cell.el_out_nodes[TOP].is_field_change) {
                            if (nodegrid.getref(x, y, z).T.el == ~0)
                                nodegrid.getref(x, y, z).T.el = curr_node_index++;
                            if (i == 1)
                                fprintf(fp, " N%u", nodegrid.getref(x, y, z).T.el);
                        }
                        if (curr_cell.core.is_th && !curr_cell.th_out_nodes[TOP].is_boundary && !curr_cell.th_out_nodes[TOP].is_field_change) {
    ...
    if (aktSim.tproV.size() > 0) {
        ...
        for (uns j = 0; j < aktSim.tproV.size(); j++) {
            ...
            if (akt_probe.oldal == CENTER) {
                ...
                fprintf(fp, " Cell_%u.%c%u", akt_cella.cell_index, curr_cell_model.el_out_nodes[akt_probe.oldal].boundary.is_X ? 'X' : 'N', curr_cell_model.el_out_nodes[akt_probe.oldal].boundary.node_index);
            }
        }
        fprintf(fp, "\n");
    }
    if (aktSim.tproT.size() > 0) {
        fprintf(fp, ".PROBE PT V circ1");
        for (uns j = 0; j < aktSim.tproT.size(); j++) {
            ...
            if (akt_probe.oldal == CENTER) {
                ...
                fprintf(fp, " Cell_%u.%c%u", akt_cella.cell_index, curr_cell_model.th_out_nodes[akt_probe.oldal].boundary.is_X ? 'X' : 'N', curr_cell_model.th_out_nodes[akt_probe.oldal].boundary.node_index);
            }
        }
        fprintf(fp, "\n");
    }
                ...
                for (uns x = 0; x < xy_n; x++, item++) {
                    ...
                    if (x < x_res && y < y_res) {
                        ...
                        if (akt_cella.cella_vector_index == ~0) // boundary cell
                            ...
                            hmg_cella& curr_cell_model = hmg_cella_vector[akt_cella.cella_vector_index];
                            if (curr_cell_model.core.is_el)
                                fprintf(fp, " Cell_%u.%c%u", akt_cella.cell_index, curr_cell_model.el_out_nodes[BOTTOM].boundary.is_X ? 'X' : 'N', curr_cell_model.el_out_nodes[BOTTOM].boundary.node_index);
                            else
                                fprintf(fp, " R0");
                        }
                    }
                    ...
                    for (uns x = 0; x < xy_n; x++, item++) {
                        ...
                        if (x < x_res && y < y_res && z < z_res) {
                            ...
                            if (akt_cella.cella_vector_index == ~0) // boundary cell
                                ...
                                hmg_cella& curr_cell_model = hmg_cella_vector[akt_cella.cella_vector_index];
                                if (curr_cell_model.core.is_el)
                                    fprintf(fp, " Cell_%u.%c%u", akt_cella.cell_index, curr_cell_model.el_out_nodes[TOP].boundary.is_X ? 'X' : 'N', curr_cell_model.el_out_nodes[TOP].boundary.node_index);
                                else
                                    fprintf(fp, " R0");
                            }
                        }
                ...
                for (uns x = 0; x < xy_n; x++, item++) {
                    ...
                    if (x < x_res && y < y_res) {
                        ...
                        if (akt_cella.cella_vector_index == ~0) // boundary cell
                            ...
                            hmg_cella& curr_cell_model = hmg_cella_vector[akt_cella.cella_vector_index];
                            if (curr_cell_model.core.is_th)
                                fprintf(fp, " Cell_%u.%c%u", akt_cella.cell_index, curr_cell_model.th_out_nodes[BOTTOM].boundary.is_X ? 'X' : 'N', curr_cell_model.th_out_nodes[BOTTOM].boundary.node_index);
                            else
                                fprintf(fp, " R1");
                        }
                    }
                    ...
                    for (uns x = 0; x < xy_n; x++, item++) {
                        ...
                        if (x < x_res && y < y_res && z < z_res) {
                            ...
                            if (akt_cella.cella_vector_index == ~0) // boundary cell
                                ...
                                hmg_cella& curr_cell_model = hmg_cella_vector[akt_cella.cella_vector_index];
                                if (curr_cell_model.core.is_th)
                                    fprintf(fp, " Cell_%u.%c%u", akt_cella.cell_index, curr_cell_model.th_out_nodes[TOP].boundary.is_X ? 'X' : 'N', curr_cell_model.th_out_nodes[TOP].boundary.node_index);
                                else
                                    fprintf(fp, " R1");
                            }
                        }
...
//***********************************************************************
void apa::write_HMG_save(FILE * fp, simulation & aktSim, uns stepIndex){
//***********************************************************************
    if (aktSim.tproV.size() > 0 || aktSim.tproT.size() > 0) {
        if (stepIndex == 0)
...
bool bouolvas(const tomb<PLString> & be,csomag & cs,bool & isel,const tomb <convection> & tconv
    ,const PLString & path, uns x_res, uns y_res, tomb<boundary> & peremlista_menteshez, bool is_intrnal){
//***********************************************************************
    ...
    if(azon=="electrical"){
        ...
        if (is_intrnal)
            throw hiba("bouolvas", "HMG internal boundary: side definition not implemented");
...
//***********************************************************************
    ...
    try{
        ...
        if(mezo==FieldEl||mezo==FieldElTherm){
            ...
            if(!bouolvas(fajl.lines()[i],normalperem,isel,tconv,path,pmodel->x_res,pmodel->y_res,peremlista_menteshez, false))
        ...
        for(;darab>0;darab--){
            ...
            while(az==az_bou){
                ...
                if(!bouolvas(fajl.lines()[i],tinner[darab-1],isel,tconv,path,pmodel->x_res,pmodel->y_res, peremlista_menteshez, true))
